...
void Balls::Move(double unit_circle_x, double unit_circle_y, float time, Platform* platform) //координаты точки на единичной окружности (направление полета шарика)
{
    ...
    if (this->GetRect().intersects(platform->GetRect())) //Если пересеклись с платформой, то проверяем где?
    {
...
bool Block::Collision(std::list<Block*>& blocks, std::list<Block*>::iterator blks, Balls& ball)
{
    ...
    {
        if ((*blks)->GetRect().intersects(ball.GetRect())) //Если шарик пересек блок, проверяем с какой стороны
        {
